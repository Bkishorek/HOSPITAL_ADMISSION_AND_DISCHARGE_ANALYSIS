...
-- Q2 Discharge rate PER 100 DAYS OF STAY
...
-- Discharge rate calculated without using subquery --
SELECT 
...
-- AVERAGE DAILY DISCHARGE (By considering total days as first admission to last discharge)
SELECT 
	ROUND(SUM(CASE WHEN OUTCOME = 'DISCHARGE' THEN 1.0 ELSE 0.0 END )/ (DATEDIFF(MAX(clean_D_O_D),MIN(clean_D_O_A))+1),0) AS Avg_daily_discharge 
...
-- Q8. Average duration of icu_stay vs outcome

SELECT AVG(`duration of intensive unit stay`) as Avg_duartion_of_icu_stay , OUTCOME FROM V_admissiondata
GROUP BY OUTCOME
ORDER BY Avg_duartion_of_icu_stay DESC;
...
-- Q10 READMISSION RATE WITHIN 30 DAYS OF DISCHARGE           
               
SELECT ROUND(SUM(Readmission_30)*100/ COUNT(*),2) AS Readmissionrate_30 FROM V_Readmission_30;
-- 4.62%

-- Q11 COUNT OF PATIENTS WITH ONLY K/C/O HTN GOT READMITTED  WITHIN 30 DAYS OF DISCHARGE 

SELECT SUM(Readmission_30) AS Readmission30_pt_with_HTN FROM V_Readmission_30 WHERE HTN = 1 AND DM = 0;
-- 167


-- Q12 COUNT OF PATIENTS WITH ONLY K/C/O DM GOT READMITTED  WITHIN 30 DAYS OF DISCHARGE
 
SELECT SUM(Readmission_30) AS Readmission30_pt_with_DM FROM V_Readmission_30 WHERE DM = 1 AND HTN = 0;

-- 74

-- Q13 COUNT OF PATIENTS WITH K/C/O DM AND HTN GOT READMITTED WITHIN 30 DAYS OF DISCHARGE 
